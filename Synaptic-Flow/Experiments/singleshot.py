...
def run(args):
    ...
    if(args.model_class == "force"):
        if(args.model == "resnet50"):
            model = load.model(args.model, args.model_class)(num_classes=num_classes,
                                            in_planes=64).to(device)
        else:
            model = load.model(args.model, args.model_class)(dataset=args.dataset).to(device)
    else:
    ...
    if(args.pre_epochs >0):
        writer_name = "runs/{}_{}_{}_{}_{}_pre".format(args.model_class,args.model,args.dataset,sparsity,args.seed)
        writer = SummaryWriter(writer_name)
    ## Pre-Train ##
    print('Pre-Train for {} epochs.'.format(args.pre_epochs))
    pre_result = train_eval_loop(model, loss, optimizer, scheduler, train_loader, 
                                 test_loader, device, args.pre_epochs, args.verbose,writer)
    ...
    if(args.post_epochs >0):
        writer_name = "runs/{}_{}_{}_{}_{}_post".format(args.model_class,args.model,args.dataset,sparsity,args.seed)
        writer = SummaryWriter(writer_name)
    ## Post-Train ##
    print('Post-Training for {} epochs.'.format(args.post_epochs))
    post_result = train_eval_loop(model, loss, optimizer, scheduler, train_loader, 
                                  test_loader, device, args.post_epochs, args.verbose,writer) 
