...
from .sparse_resnet import  resnet50 as sparse_resnet_50
network_name_module = {
    'resnet34': resnet34,
    'resnet50': resnet50,
    'resnet110': resnet110,
...
def sparse_resnet_cifar(network_name, dataset_name,stable_resnet=False, in_planes=64):
    print(network_name)
    num_classes = dataset_num_classes[dataset_name]
    network_name = network_name.split('stable')[-1]

    net = sparse_resnet_50(num_classes=num_classes, stable_resnet=stable_resnet,
                                            in_planes=in_planes)
    return net


def sparse_resnet_cifar_experiment( net, optimiser_name="sgd"):
    """
    Util function to generate necessary components to train resnet network
    on CIFAR10/100 datasets.
    """
    ...
    return  optimiser, scheduler, loss, EPOCHS

def train_cross_entropy(epoch, model, train_loader, optimizer, scheduler,device, writer, LOG_INTERVAL=20):
    '''
