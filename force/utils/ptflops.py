...
def get_model_complexity_info(model, input,
...
def get_model_parameters_number(model):
    ...
    for m in model.modules():
        if hasattr(m, 'bias'):
            if(type(m.bias) !=  type(None) and m.bias.requires_grad):
                params_num += m.bias.numel()
        if hasattr(m, 'weight'):
            if(type(m.weight) != type(None) and m.weight.requires_grad):
                if(isinstance(m, nn.Conv2d)) or isinstance(m, nn.Linear):
                    params_num += (m.weight.data != 0.).int().sum().item()
                else:
                    params_num += m.weight.numel()
        # print(layer.weight)
        # exit(0)
        # if(isinstance(layer, nn.Conv2d)) or isinstance(layer, nn.Linear):
        #     params_num = (layer.weight.data != 0.).float().sum()
        
    # for layer, p in model.named_parameters():
    #     print(name)
    #     if p.requires_grad:
    #         if("feature" in name and "weight" in name):
    #             params_num += nonzero(p).sum()
    #         else:
    #             params_num += p.numel()

    # params_num = sum(p.numel() for p in model.parameters() if p.requires_grad)
    # params_num = sum(nonzero(p).sum() for p in model.parameters() if p.requires_grad)
    return params_num
    ...
    def add_flops_counter_hook_function(module, ost, verbose, ignore_list):
        if type(module) in ignore_list:
            ...
            if type(module) in CUSTOM_MODULES_MAPPING:
                ...
                # if(type(module) in BACKWARD_MAPPING):
                #     handle = [handle, module.register_backward_hook(BACKWARD_MAPPING[type(module)])]
...
def linear_flops_counter_hook(module, input, output):
    ...
    sparsity = nonzero(weight).sum() / np.prod(weight.shape)

    # input_np = input.detach().cpu().numpy().copy()
    ...
    module.__flops__ += int(np.prod(input.shape) * output_last_dim) *  sparsity + bias_flops


def linear_flops_counter_hook_backward(module, input, output):
    input = input[0]
    output = output[0]
    # pytorch checks dimensions, so here we don't care much
    output_last_dim = output.shape[-1]
    bias_flops = output_last_dim if module.bias is not None else 0
    weight = module.weight.detach().cpu().numpy().copy()
    module.__flops__ += int(np.prod(input.shape) * output_last_dim) * nonzero(weight).sum() / np.prod(weight.shape) + bias_flops
...
def img2col(input,k,stride = (1,1)):
    if isinstance(stride,int):
        stride = (stride,stride)
    input_windows = input.unfold(2, k, stride[0]).unfold(3, k, stride[1])
...
def padding(input,padding):
    padding_value = 0
    out_dims = input.shape[0],input.shape[1],input.shape[2]+padding[0],input.shape[3] +padding[1]
    out_tensor = input[0].data.new(*out_dims).fill_(padding_value)
    batch_first = True
    for i, tensor in enumerate(input):
        ...
        if batch_first:
            out_tensor[i, :, :width,:height] = tensor
        else:
            out_tensor[:, i, :width,:height] = tensor
    return out_tensor
...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    weight = conv_module.weight.detach().cpu().numpy().copy()
    weight = weight.reshape(weight.shape[0],-1)

    sparsity = nonzero(weight).sum() / np.prod(weight.shape)

    # input_np = img2col(padding(input,conv_module.padding),weight.shape[-1],conv_module.stride)
    # input_np = input_np[0,:,:].transpose()
    ...
    # ax2.set_aspect('auto')
    # plt.show()
    overall_flops = overall_conv_flops* sparsity + bias_flops
...
def remove_flops_counter_hook_function(module):
    if is_supported_instance(module):
        if hasattr(module, '__flops_handle__'):
            if(type(module.__flops_handle__) == list):
                for i in module.__flops_handle__:
                    i.remove()
            else:
